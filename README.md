...
which is kept for reference under [`isola/`](isola/) (see below).
...
```
src/
  main.tsx                    entry point for the simulator (index.html)
  deckbuilder-main.tsx        entry point for the deck builder (deckbuilder.html)
  App.tsx                     simulator root: menu/boot/play screen switch, engine bootstrap

  engine/                     game rules, ported from the original engine/browser/src
    duel.ts                   THE ADAPTER — wraps ocgcore, emits generic events (move,
                               summon, chain, attack, damage, phase...)
    autopilot.ts               drives a duel programmatically (used by bots/tests)
    trivial.ts                 auto-resolves trivial forced decisions
    ai/
      brain.ts                 bot logic — one "clean" brain, lower difficulties are the
                                same brain with specific, measurable flaws
      knowledge.ts              card/board knowledge the AI reasons over
      evaluate.ts                board/position evaluation heuristics
      view.ts                    AI's read of the generic duel events

  game/
    gameEngine.ts              orchestrator: engine + adapter + AI, no DOM — port of the
                               original main.js. Exposes raw state (not CSS), consumed by
                               React through useSyncExternalStore
    useGameEngine.ts           React hook wrapping GameEngine with useSyncExternalStore

  components/
    simulator/                 board, hand, HUD, prompts, modals, menu screens
      Board.tsx, Card.tsx, DuelScreen.tsx, Hud.tsx, Modals.tsx, PromptPanel.tsx
      menu/                     Home, Play, Bots, Options screens + saved config
    deckbuilder/
      DeckBuilderApp.tsx        the deck builder UI

  render/
    layout.ts                  board/card transform + position math (kept out of
                               gameEngine.ts on purpose — see file header)
  ...
  data/                        card database, legal pool (goat-pool.json), copy limits
                               (goat-limites.json), decks (mazos.json), avatars,
                               pool_cards.json / pool_texts.json
  vendor/                      ocgcore.bundle.js (wasm engine) + scripts.bundle.js
                               (bundled Lua card scripts), cards/names subsets, cardback
  types/                       cards.ts, ocgcore.ts — shared TypeScript types

tests/
  engine/                      duel.test.ts, ai.test.ts, cards.test.ts, i18n.test.ts,
                               rules.test.ts
  game/gameEngine.test.ts
  render/                      App.test.tsx, DuelScreen.test.tsx
  support/scenario.ts          builds an exact board state (specific cards in field/
                               hand/graveyard/deck), drives it with a script, and queries
                               ocgcore directly — used to pin down rulings on specific
                               card interactions
  setup.ts                     vitest/jsdom setup

isola/                         the ORIGINAL project this was ported from: a
                               self-contained, no-build vanilla-JS/HTML version.
                               Kept as a reference and historical record — not built
                               or run as part of this app. See isola/README.md.
...
original project (see `isola/README.md`), now split across `engine/`, `game/`
...
specific, measurable flaws (see `isola/README.md` for the methodology and
